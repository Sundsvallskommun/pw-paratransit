...
import static se.sundsvall.paratransit.util.ErrandUtil.getOptionalStakeholder;
...
public class VerifyResidentOfMunicipalityTaskWorker extends AbstractWorker {

	static final String MAIN_ADDRESS_TYPE = "POPULATION_REGISTRATION_ADDRESS";

	private final CitizenClient citizenClient;

	VerifyResidentOfMunicipalityTaskWorker(final CamundaClient camundaClient, final CaseDataClient caseDataClient, final FailureHandler failureHandler,
		final CitizenClient citizenClient) {
	 ...
	 *                        citizen, address, etc).
	 */
	private Optional<String> getMunicipalityId(final String municipalityId, final String personId) {
		if (isBlank(personId)) {
			return empty();
		}
