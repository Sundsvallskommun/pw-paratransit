...
import static se.sundsvall.paratransit.Constants.CAMUNDA_VARIABLE_APPLICANT_NOT_RESIDENT_OF_MUNICIPALITY;
...
import static se.sundsvall.paratransit.businesslogic.worker.actualization.VerifyResidentOfMunicipalityTaskWorker.MAIN_ADDRESS_TYPE;
...
import se.sundsvall.paratransit.integration.citizen.CitizenClient;
...
class VerifyResidentOfMunicipalityTaskWorkerTest {
	...
	void executeForExistingCitizenThatBelongsToCorrectMunicipality() {
		...
		when(errandMock.getStakeholders()).thenReturn(List.of(
			createStakeholder(null, ROLE_REPORTER, "Reporter", "Lady"),
			createStakeholder(PERSON_ID.toString(), ROLE_APPLICANT, "Mr", "Applicant"),
			createStakeholder(null, ROLE_REPORTER, "Reporter", "Dude")));
	...
	private Stakeholder createStakeholder(final String personId, final String role, final String firstName, final String lastName) {
	...
	private CitizenExtended createCitizen(final UUID personId, final String municipalityId) {
